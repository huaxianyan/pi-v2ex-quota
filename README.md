...
1. **状态栏显示**：额度进度条、剩余百分比、距窗口重置的倒计时（最后一分钟逐秒走，归零改说 `即将开始`），外加两项开关的当前状态。
...
一行分成两段，中间用 ` | ` 隔开：前半是配额，后半是两个开关的当前状态。

```
V2EX ███████░ 87% · 4h12m | 续跑 关 · 重试 关      还剩 87%，距重置 4 小时 12 分
V2EX ░░░░░░░░ 0% · 2h41m | 续跑 开 · 重试 关      配额用尽，还没排入等待
V2EX 等待 2h41m | 续跑 开 · 重试 关               已排入自动续跑，倒计时结束后继续
V2EX 等待 43s | 续跑 开 · 重试 关                 只剩不到一分钟，改成按秒走
V2EX 即将开始 | 续跑 开 · 重试 关                 倒计时归零，正在复核并注入续跑
V2EX 重试 45s | 续跑 开 · 重试 开                 上游故障，等退避后重试（与剩余额度无关）
V2EX 即将重试 | 续跑 开 · 重试 开                 重试的倒计时归零，下次轮次马上开始
V2EX 空闲 | 续跑 关 · 重试 关                     没有有效窗口，发下一条消息时开新窗口
V2EX -- | 续跑 关 · 重试 关                       还没取到数据
```

**配额段**：进度条与百分比都表示**剩余**额度（不是已用），后面的时间是距窗口重置（重试中则是距下次重试）。进度条八格，实心是剩余、空心会压暗一档；只有真正满额才画满格，否则 97% 也会画成满格，比旁边的数字还误导。

**倒计时**：一分钟以上按分钟报（跟 `pollSeconds` 的轮询一起走），进了最后一分钟改成每秒一动 —— 那时候按分钟级刷新的话，屏幕上就是一个不动的数字。剩余不足一秒（含已经到点但复核还没回来）时改说 `即将开始`（重试那条路说 `即将重试`），不再显示一个不动的 `0s`。这个倒计时纯粹是本地按排期时刻算的，不追求跟服务端对齐：到点之后还要复核额度、注入消息，本来就会晚一点。

**开关段**：`续跑` 与 `重试` 分别对应 `/v2ex wait`、`/v2ex retry`。关着的也照常显示 —— 状态栏本来就是为了不用再敲一条命令去确认。
...
**pi 的「working」指示会消失，这是正常的**：`ctx.abort()` 之后本轮就已经结束了，没有任何东西在跑，等待是由扩展自己的定时器承担的，跟 agent 的运行状态无关。它还在等你的依据是状态栏的 `V2EX 等待 <倒计时>`（最后一分钟里逐秒在走，归零后变成 `V2EX 即将开始`），以及排期时收到的那两条通知。
...
配额查询与对话请求走的是**同一个代理**，也就是 pi 的全局出网设置 —— 扩展不再自己维护一份代理配置。理由很直白：这两件事要的是同一个代理，而 pi 已经把这件事管好了，自己再配一处只会变成「两处都得记得改」。

pi 启动时会把 `globalThis.fetch` 换成带 `EnvHttpProxyAgent` 的 undici 版，而扩展的配额查询用的就是同一个 `globalThis.fetch`。所以**一个设置同时覆盖两条通道**：

- `HTTP_PROXY` / `HTTPS_PROXY` 环境变量（`EnvHttpProxyAgent` 认它们），或
- `~/.pi/agent/settings.json` 里的 `httpProxy`（pi 启动时用 `??=` 落进上面两个环境变量）
...
**写一次就持久，不必每次开终端都 `export`** —— 这正是它比环境变量省事的地方。要排除某些域名用 `NO_PROXY`（逗号分隔，支持域名后缀与 IP 段）。

下面四条是本机读 pi 源码 + 实测确认的，踩中任何一条都会表现成「设了没用」：

- **改完要重启 pi 才生效，`/reload` 不够。** pi 启动时 `applyHttpProxySettings()` 把这个值用 `??=` 落进 `process.env.HTTP_PROXY` / `HTTPS_PROXY`，紧接着 `configureHttpDispatcher()` 建一个 undici `EnvHttpProxyAgent` 当全局 dispatcher。而那个 agent 是在**构造函数里**读环境变量的，建完就固定；`/reload` 只重装 dispatcher、不重跑写环境变量那一步，所以热改不生效。
- **环境变量优先。** 落值用的是 `??=`，只在环境变量为空时才写。你 shell 里那份 `HTTPS_PROXY` 不撤掉，settings.json 里写什么都不起作用 —— 两边并存时生效的是 shell 那份。
- **只认 agent 目录的设置。** pi 的文档写明 `httpProxy`「can only be set in agent-directory settings」，项目级 `.pi/settings.json` 里写无效。
- **它不是按 provider 分流的。** pi 没有 provider 级代理字段，`httpProxy` 只有全局一份，`NO_PROXY` 只能反向排除。所以切模型时不必改来改去，但也做不到「只让某一个 provider 走代理」。

### 为什么扩展不自己管代理

早先的版本试过：`/v2ex proxy` 用 `node:net` 手搭一片隧道（http CONNECT / socks5 / socks4a），零依赖，只作用于配额查询那一次请求，还带协议自动探测。后来整块去掉了，两个理由：

1. **它原本要解决的事做不到。** 那套东西的出发点是「让整个 API 走代理」，可它只作用于扩展自己发的那一次配额查询 —— pi 的对话请求另有其道，一个开关管不住两条通道。
2. **但只管配额查询又没必要。** 既然 pi 的全局 fetch 本来就带代理，扩展再搭一套手搓隧道就是重复劳动，还多一份要写、要测的握手指令。

代价只有一条：**配额查询从此依赖「启动 pi 时那两条环境变量在，或 `httpProxy` 已设」**。哪次忘了设、本机又直连不通，状态栏会停在 `V2EX --`（`/v2ex debug on` 能看到 `quota fetch failed`）。这是一条清楚、可预期的依赖，比长期维护一套代理实现划算得多。
...
- **秒级倒计时不靠轮询**。`pollSeconds` 默认 60 秒，等它还差一分钟时按分钟级刷新的话，屏幕上就是一个不动的数字。所以排期之后会另挂两个定时器：一个在「进入最后一分钟」那一刻到点（等待排在一分钟以外时不逐秒空转），一个在最后一分钟里每秒刷一次状态栏。两个都是 unref 的，并且计划一被收走（续跑注入、用户取消）就自己停掉。它只是展示用的本地倒计时，不追求与服务端对齐。
...
倒计时那几条用例要留意机器负载：跑全量时各测试文件是并行进程，`await sleep(80)` 实测能被拖到 1.6 秒。所以别断言「排期那一刻余量正好是 X 秒」这种会把调度延迟算进去的值，而是放宽到形状（`等待 \ds`）并留足余量；「逐秒在走」则靠 `statusLog` 里出现了几个不同的秒数来判断 —— 会话的轮询间隔被设成 3600 秒，那些写入只可能来自倒计时的定时器。
...
### 真机自检：续跑、重新排期、手动排队与上游重试

`npm test` 用的是假 pi，证明的是扩展自身的逻辑。而真正依赖 pi 生命周期的那几件事 —— `sendUserMessage` 在**会话空闲**时会不会开新一轮、用户接管后等待会不会重新排上、`/v2ex start` 能不能在零 agent 轮次下排期、待续跑的最后一分钟里状态栏会不会逐秒走并在到点改口、上游故障之后能不能按退避接着跑 —— 只有真 pi 能回答。这些由 `tools/verify-resume.mjs` 覆盖：
...
npm run verify:start-live      # /v2ex start 排上后，倒计时与到点续跑能不能真的接上
...
`V2EX_VERIFY_PROXY` 是给 pi 子进程设 `HTTP_PROXY` / `HTTPS_PROXY` 的 —— pi 启动时把 `globalThis.fetch` 换成读这两个变量的 undici 版，扩展的配额查询用的就是同一个 fetch，所以配一次，配额查询与对话请求两条都走代理。**扩展自己不再有代理配置项。** 另外三条打的是 `127.0.0.1` 上的假服务，不要设这个变量，免得代理自己的 bypass 规则插一脚。
...
`start-live` 是 `start` 与 `resume` 的合体 —— 入口是命令、出口是新的 agent 轮次。**两条分开验过不等于链路成立**，所以专门跑一次：用假配额服务把「窗口重置」压到 8 秒后（真窗口要等几十分钟到几小时），一分钟内跑完全程。8 秒的等待整段落在最后一分钟里，正好把倒计时也一起验了（会话的轮询间隔是 60 秒，这段时间里状态栏上的秒数只可能来自倒计时自己）。实测输出（2026-09-23，状态栏当时还有一项代理，格式保留原样）：
